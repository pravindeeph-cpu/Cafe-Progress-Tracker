...
function seedTable(table, rows) {
  ...
  if (countRow.c > 0) return { table, inserted: 0, skipped: true };
...
const licences = [
  // Mandatory for every food & beverage outlet
  ...
  // Conditional - only relevant if flagged applicable
...
seedTable('skill_prep', skillPrep);
...
seedTable('pre_opening_checklist', preOpening);

// ---------- Go/No-Go Gates ----------
const gateStmt = db.prepare('SELECT COUNT(*) as c FROM gates').get();
if (gateStmt.c === 0) {
  const insertGate = db.prepare('INSERT INTO gates (name, sort_order) VALUES (?, ?)');
  const insertItem = db.prepare('INSERT INTO gate_items (gate_id, item, complete, notes) VALUES (?, ?, 0, ?)');
  ...
  const insertAll = db.transaction((defs) => {
    defs.forEach((gate, idx) => {
      const info = insertGate.run(gate.name, idx);
      const gateId = info.lastInsertRowid;
      for (const item of gate.items) insertItem.run(gateId, item, '');
    });
  });
  insertAll(gateDefs);
}
